...
class NMRResultsPanel(ResultsPanel[NMRResultsModel]):
    ...
    def _render_tensor_components(self):
        ...
        tensor_display = ipw.HTML()

        def update_tensor_display(change):
            atom_label = change["new"]
            if atom_label not in self._model.chemical_shift_tensors:
                tensor_display.value = "<p>No tensor data available for this atom.</p>"
                return
            ...
            # Add eigenvalues (principal components)
            ...
            tensor_display.value = html

        atom_selector.observe(update_tensor_display, names="value")

        # Initialize with first atom
        if self._model.atom_labels:
            atom_selector.value = self._model.atom_labels[0]
    ...
    def _render_structure_view(self):
        ...
        structure_viewer = StructureDataViewer(self._model.structure)

        # Extract atom indices from labels (e.g., "Si1" -> 0 for 1-based indexing)
        highlighted_atoms = []
        for atom_label in self._model.atom_labels:
            # Parse the label to get the index
            # Format is typically: ElementSymbol + Index (1-based)
            import re
            match = re.match(r"([A-Za-z]+)(\d+)", atom_label)
            if match:
                index = int(match.group(2)) - 1  # Convert to 0-based
                highlighted_atoms.append(index)

        if highlighted_atoms:
            structure_viewer.displayed_selection = highlighted_atoms

        return ipw.VBox([
            ipw.HTML("<h4>Structure with NMR-Active Sites</h4>"),
            ipw.HTML("<p>Highlighted atoms are those for which NMR shielding was computed.</p>"),
            structure_viewer,
        ])
